#https://github.com/kirbyloco/gamer_autosignin
...
def _autosign(sess):
    ...
    if 'data' in jsoninfo:
        logging.info(f'巴哈姆特自動簽到成功!!\n已簽到第 {str(jsoninfo["data"]["days"])} 天')
    else:
        logging.warning('簽到失敗')
        PushMessage("gamer check failed.", "簽到失敗")
...
def _autoanswer(sess):
    ...
    if token is None:
        logging.warning('Missing token. Body: {}'.format(tokenweb.json()))
        PushMessage("gamer check failed.", "Missing token.")
        return
    data = {
        'token': token,
        'ans': ans,
    ...
    jsoninfo = c.json()
    if jsoninfo['ok'] == 1:
        logging.info(jsoninfo['gift'])
    else:
        logging.warning('答案錯誤')
        PushMessage("gamer check failed.", "答案錯誤")
...
def GamerCheck(account, password):
    data = {
        'uid': account,
        'passwd': password,
        'vcode': '7045'
    ...
    try:
        _login(data)
        logging.info("GamerCheck finish.")
    except Exception as e:
        logging.error("Exception: %s", traceback.format_exc())
        PushMessage("gamer check failed.", "gamer check failed.")
